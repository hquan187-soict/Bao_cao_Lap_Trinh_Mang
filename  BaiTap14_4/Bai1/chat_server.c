...
static void handle_client_data(int fd) {
  ...
  if (nbytes <= 0) {
    if (clients[idx].state == STATE_CHATTING) {
      ...
      snprintf(notice, sizeof(notice),
               "[%s]%s (%s) đã rời khỏi phòng chat\n", ts,
               clients[idx].name, clients[idx].id);
      broadcast(fd, notice);
      printf("'%s' (%s) disconnected.\n", clients[idx].id,
             clients[idx].addr);
    } else {
      printf("Client từ %s disconnected (chưa đăng ký).\n",
             clients[idx].addr);
    }
    remove_client(idx);
    return;
  ...
  if (clients[idx].state == STATE_WAIT_ID) {
    ...
    if (!parse_register(buf, id, name)) {
      send_str(fd, "Cú pháp sai! \n");
      return;
    }
    if (find_client_by_id(id) >= 0) {
      send_str(fd, "ID này đã được sử dụng!\n> ");
      return;
    }
    ...
    snprintf(welcome, sizeof(welcome), "%s (%s) connected!\n", name, id);
    send_str(fd, welcome);
    char ts[32];
    char notice[BUFFER_SIZE];
    get_timestamp(ts, sizeof(ts));
    snprintf(notice, sizeof(notice),"[%s]%s (%s) đã tham gia phòng chat\n", ts, name, id);
    broadcast(fd, notice);

    printf("Client '%s' (%s) đã đăng ký từ %s.\n", id, name,clients[idx].addr);
    return;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (port <= 0 || port > 65535) {
    fprintf(stderr, "Port không hợp lệ: %s\n", argv[1]);
    exit(EXIT_FAILURE);
  }
    ...
    if (FD_ISSET(server_fd, &read_fds)) {
      ...
      if (new_fd < 0) {
        ...
        if (add_client(new_fd, ip) < 0) {
          send_str(new_fd, "Server đầy! Vui lòng thử lại sau.\n");
          close(new_fd);
          printf("Từ chối kết nối từ %s.\n", ip);
        } else {
          printf("Kết nối mới từ %s (fd=%d).\n", ip, new_fd);
          send_str(new_fd, "Vui lòng đăng ký theo cú pháp: client_id: client_name\n");
        }
      }
    }
